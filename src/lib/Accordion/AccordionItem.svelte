...
<div class="accordion-item {classesBase}" data-testid="accordion-item">
	<!-- Control -->
	<button type="button" class="accordion-control {classesControl}" id="accordion-control-{id}" on:click={setActive} on:click on:keydown on:keyup on:keypress aria-expanded={openState} aria-controls="accordion-panel-{id}">
		<!-- Lead -->
		{#if $$slots.lead}
			<div class="accordion-lead {$$props.class}">
				<slot name="lead" />
			</div>
		{/if}
		<!-- Summary -->
	...
	<!-- Panel -->
	{#if openState}
		<div class="accordion-panel {classesPanel}" id="accordion-panel-{id}" transition:slide|local={{ duration }} role="region" aria-hidden={!openState} aria-labelledby="accordion-control-{id}">
